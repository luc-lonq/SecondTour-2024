    ...
    for candidat in candidat_ordened:
        ...
        for x in range(0, 2):
            ...
            for jour_debut_preparation_voulue in range(1, 4):
                ...
                while heure_debut_preparation_voulue + temps_passage_matiere + temps_preparation_matiere <= timedelta(hours=13):
                    ...
                    for a_salle in salle_matiere:
                        ...
                        for creneau in all_creneaux:
                            if creneau["debut_preparation"].day == jour_debut_preparation_voulue:
                                ...
                                delta_m30 = (
                                    debut_preparation_creneau - timedelta(minutes=10))
                                ...
                                if aucune_collision:
                                    ...
                                    for creneau_test in all_creneau_test_break:
                                        if (res := (creneau_test["debut_preparation"] - (creneau_prec["debut_preparation"] + timedelta(minutes=10)))) >= timedelta(minutes=0):
                                            break_time += res.seconds / \
                                                3600 + (res.seconds % 3600)
                                        creneau_prec = creneau_test
                                        x += 1
                                        if x >= 4:
                                            if break_time == 0 and creneau_prec["debut_preparation"] + timedelta(minutes=10) == heure_debut_preparation_voulue:
                                                x = 0
                                                aucune_collision = False
